...
function ADimpotieren {
    ...
    foreach ($obj in $Global:schueler.SelectNodes("//ns:Obj", $namespace)) {
        ...
        if (-not (Get-ADGroup -Filter "Name -eq '$classGroup'")) {
            New-ADGroup -Name $classGroup -GroupScope Global -Path "$Global:OUKlasse" -ErrorAction SilentlyContinue
        ...
        if (-not (Get-ADGroup -Filter "Name -eq '$classGroup2'")) {
            New-ADGroup -Name $classGroup2 -GroupScope Global -Path "$Global:OUKlasse" -ErrorAction SilentlyContinue
